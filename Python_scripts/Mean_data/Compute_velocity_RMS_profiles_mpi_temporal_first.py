...
OUTPUT_DATA_NAME = "AoA5_Re50000_velocity_RMS_profiles_data_mpi_temporal_first_2.h5"
...
if rank == 0:
    ...
    if global_snapshot_count > 0:
        ...
        if rank == 0:
            print(f"Temporal averages computed. 3D field shape: {u_sq_temporal_avg.shape}")
        ...
        for prof_idx, prof_loc in enumerate(profile_locations):
            ...
            u_sq_3d = u_sq_temporal_avg[:, j_idx, i_idx]
            v_sq_3d = v_sq_temporal_avg[:, j_idx, i_idx]
            w_sq_3d = w_sq_temporal_avg[:, j_idx, i_idx]
            
            # Spatial average over z-direction
            u_sq_profile = np.mean(u_sq_3d, axis=0)
            v_sq_profile = np.mean(v_sq_3d, axis=0)
            w_sq_profile = np.mean(w_sq_3d, axis=0)
            
            # Compute RMS
            u_rms = np.sqrt(u_sq_profile)
            v_rms = np.sqrt(v_sq_profile)
            w_rms = np.sqrt(w_sq_profile)
