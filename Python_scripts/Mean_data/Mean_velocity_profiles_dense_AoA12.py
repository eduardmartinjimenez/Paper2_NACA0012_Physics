...
# Figure output path
FIGURE_OUTPUT_PATH = "/home/jofre/Members/Eduard/Paper2/Figures/"
...
# VISUALIZATION PLOTS
# =======================
def build_2d_grid_from_profiles(profiles, rms_by_xc, u_infty, n_interp_y=100):
    """Build 2D grid of u_rms from wall-normal profiles.
    
    Args:
        profiles: List of profile dictionaries
        rms_by_xc: Dictionary mapping x/c to RMS profile data
        u_infty: Reference velocity
        n_interp_y: Number of y points for interpolation
        
    Returns:
        x_grid, y_grid, u_rms_grid: 2D grids for contour plotting
    """
    # Collect x and y ranges
    x_list = []
    y_list = []
    u_rms_list = []
    ...
    for profile in profiles:
        ...
        rms_profile = rms_by_xc.get(x_c, None)
        
        if rms_profile is None:
            continue
            
        # Get wall-normal distance
        y_vals = profile["y_prime"]
        x_val = profile["x_start"]
        
        # Get RMS values and normalize
...
# Build 2D grid from RMS profiles
X_grid, Y_grid, U_rms_grid = build_2d_grid_from_profiles(
    wall_normal_profiles_dense, rms_by_xc, u_infty, n_interp_y=150
)
...
if X_grid is not None:
    ...
    # Match axis tick label sizes to colorbar
    ...
    for x_c_target in x_c_locations_original:
        ...
        # Plot the velocity profile (mean)
        x_profile = profile["x_start"] + u_norm_sorted * scale_factor
        y_profile = profile["y_start"] + eta_sorted
    ...
    print("Could not generate u_rms contour plot - RMS data unavailable")
