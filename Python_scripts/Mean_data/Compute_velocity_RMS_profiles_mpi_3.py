...
OUTPUT_DATA_NAME = "AoA5_Re50000_velocity_RMS_profiles_data_mpi_3_all.h5"
...
local_mean_u_2d = np.zeros((ny, nx))
local_mean_v_2d = np.zeros((ny, nx))
local_mean_w_2d = np.zeros((ny, nx))
local_mean_u2_2d = np.zeros((ny, nx))
local_mean_v2_2d = np.zeros((ny, nx))
local_mean_w2_2d = np.zeros((ny, nx))
...
for idx, file in enumerate(local_files):
    ...
    local_mean_u_2d += np.mean( u_rot, axis=0 )
    local_mean_v_2d += np.mean( v_rot, axis=0 )
    local_mean_w_2d += np.mean( w_rot, axis=0 )
    local_mean_u2_2d += np.mean( u_rot**2, axis=0 )
    local_mean_v2_2d += np.mean( v_rot**2, axis=0 )
    local_mean_w2_2d += np.mean( w_rot**2, axis=0 )
...
global_mean_u_2d = comm.reduce(local_mean_u_2d, op=MPI.SUM, root=0)
global_mean_v_2d = comm.reduce(local_mean_v_2d, op=MPI.SUM, root=0)
global_mean_w_2d = comm.reduce(local_mean_w_2d, op=MPI.SUM, root=0)
global_mean_u2_2d = comm.reduce(local_mean_u2_2d, op=MPI.SUM, root=0)
global_mean_v2_2d = comm.reduce(local_mean_v2_2d, op=MPI.SUM, root=0)
global_mean_w2_2d = comm.reduce(local_mean_w2_2d, op=MPI.SUM, root=0)
...
if rank == 0:
    ...
    if global_snapshot_count > 0:
        ...
        if rank == 0:
            print(f"Temporal averages computed. 3D field shape: {global_mean_u_2d.shape}")
        ...
        for prof_idx, prof_loc in enumerate(profile_locations):
            ...
            mean_u = global_mean_u_2d[ j_idx, i_idx ]/global_snapshot_count
            mean_v = global_mean_v_2d[ j_idx, i_idx ]/global_snapshot_count
            mean_w = global_mean_w_2d[ j_idx, i_idx ]/global_snapshot_count
            
            mean_u2 = global_mean_u2_2d[ j_idx, i_idx]/global_snapshot_count
            mean_v2 = global_mean_v2_2d[ j_idx, i_idx]/global_snapshot_count
            mean_w2 = global_mean_w2_2d[ j_idx, i_idx]/global_snapshot_count
