...
# Using the same approach as in compute_wall_units.py:
# 1. Decompose velocity into normal and tangential components
# 2. Compute wall shear stress as: tau_w = mu * U_tangential / distance_to_wall
...
# Compute signed wall shear stress: tau_w = mu * (dU_t/dn) ≈ mu * U_t / delta_n
# where delta_n is the distance from the wall to the first fluid point
tau_wall = mu_ref * U_t_signed / proj_distances  # (N,)
...
print("\n" + "="*60)
print("SKIN FRICTION COEFFICIENT STATISTICS")
print("="*60)
...
with h5py.File(SAVE_PATH, "w") as f:
    ...
    f.create_dataset("U_t_signed", data=U_t_signed)
