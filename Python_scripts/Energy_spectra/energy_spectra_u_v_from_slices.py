"""
Energy Spectra from Slice Data - Refactored v2
====================================================

Compute raw periodograms of streamwise and cross-stream velocity fluctuations
from slice snapshot data at physically-defined probe locations.

Key improvements (v2):
- Numeric file sorting (not lexicographic)
- All available spanwise data z
- Correct physical time step computation
- Clean normalization with variance validation
- Nondimensional frequency output
- Multiplicative plot offsets (not additive)
- Structured HDF5 output
- Explicit slice structure verification
- Requested vs actual probe location visualization
- rfft-based one-sided spectrum (cleaner than manual fft)
- Multi-slice consistency (slice_id inferred from path)
"""
...
# ============================================================================
# VERIFY SLICE STRUCTURE (Phase 2)
# ============================================================================
...
if x_rel_std > 1e-6:  # Relative tolerance for floating point (more realistic)
...
# ============================================================================
# SORT AND VALIDATE SNAPSHOT FILES (Phase 7: infer slice_id)
# ============================================================================
...
# Compute physical time step
dt_save = delta_iter * dt_iteration
fs = 1.0 / dt_save  # Sampling frequency
...
# ============================================================================
# VISUALIZE AIRFOIL AND PROBE LOCATIONS (Phase 9: improved markers)
# ============================================================================
...
plt.tight_layout()
# probe_viz_file = os.path.join(SAVE_DIR, f"airfoil_probe_locations_{slice_id}.png")
# plt.savefig(probe_viz_file, dpi=150, bbox_inches='tight')
# print(f"Probe visualization saved: {probe_viz_file}")
plt.show()

# ============================================================================
# EXTRACT TIME SERIES (ALL Z DATA) - Phase 3: track n_samples
# ============================================================================
...
# Verify all probes have consistent time samples and compute actual n_samples
# (Check list lengths before converting to arrays)
...
# ============================================================================
# COMPUTE ENERGY SPECTRA (Phase 4: rfft) (Phase 5: variance checks)
# ============================================================================
...
# One-sided frequency vector using rfft (cleaner than manual fft)
...
for j_idx, y_val in zip(y_locations_idx, y_locations_val):
    ...
    for iz in range(nz):
        ...
        # For even n_samples: returns n_samples//2 + 1 values (includes DC and Nyquist)
        # For odd n_samples: returns (n_samples+1)//2 values (includes DC, no Nyquist)
        U_rfft = np.fft.rfft(u_fluct)  # Returns (nfreq,) complex
        V_rfft = np.fft.rfft(v_fluct)

        # One-sided periodogram normalization:
        # For a one-sided spectrum, we multiply by 2 to account for negative frequencies
        # DC (f=0) and Nyquist (f=fs/2, only if even n_samples) are unique and counted once
        E_uu_z[:, iz] = (2.0 * dt_save / n_samples) * (np.abs(U_rfft) ** 2)
        E_vv_z[:, iz] = (2.0 * dt_save / n_samples) * (np.abs(V_rfft) ** 2)

        # Correct for DC and Nyquist (appear once, not twice, so divide by 2)
        E_uu_z[0, iz] /= 2.0    # DC component
        E_vv_z[0, iz] /= 2.0

        if n_samples % 2 == 0:
            E_uu_z[-1, iz] /= 2.0  # Nyquist (only if even n_samples)
            E_vv_z[-1, iz] /= 2.0
    ...
    # Phase 5: Compute relative errors (per z and z-averaged)
...
# Print variance validation for first two probes
...
# ============================================================================
# CREATE SPECTRAL PLOTS (MULTIPLICATIVE OFFSETS FOR VISUALIZATION)
# ============================================================================
...
for plot_idx, sorted_idx in enumerate(sorted_indices):
    ...
    # Multiplicative offset: E_plot = E * 10^(offset * scaling_factor)
    offset_factor = plot_idx * 0.4  # Adjust spacing
...
ax.set_title(f"Streamwise Velocity Energy Spectrum ({slice_id})\nAOA={AOA_deg}°, x={slice_x:.4f}",
             fontsize=13, fontweight='bold')
...
ax.set_title(f"Cross-stream Velocity Energy Spectrum ({slice_id})\nAOA={AOA_deg}°, x={slice_x:.4f}",
             fontsize=13, fontweight='bold')
ax.legend(loc='upper right', fontsize=10)
ax.grid(True, which='both', alpha=0.3)

plt.tight_layout()
spectra_file = os.path.join(SAVE_DIR, f"energy_spectra_uv_{slice_id}.png")
plt.savefig(spectra_file, dpi=150, bbox_inches='tight')
print(f"Spectra plot saved: {spectra_file}")
plt.show()

# ============================================================================
# SAVE RESULTS TO HDF5 (Phase 8: improved structure)
# ============================================================================
...
output_file = os.path.join(SAVE_DIR, f"energy_spectra_data_{slice_id}.h5")
...
print("\n" + "="*70)
print("SUMMARY OF ENERGY SPECTRA ANALYSIS")
print("="*70)
...
print(f"\nOutput files:")
# print(f"  Probe visualization: {probe_viz_file}")
