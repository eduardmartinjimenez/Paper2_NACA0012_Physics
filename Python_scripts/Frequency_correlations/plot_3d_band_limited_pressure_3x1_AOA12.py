...
unconditional wall-pressure correlation 3x1 panel figure, but uses the frequency-
...
VIZ_YLIM_OFFSET = [0.1, 0.3]  # Extended y-window to show airfoil surface
    ...
    for correlation_type in CORRELATION_TYPES:
        for band in BANDS:
            ...
            output_png = os.path.join(
                OUTPUT_DIR,
                f"band_limited_pressure_u_{correlation_type}_{band}_3x1panel_AOA{AOA}.png",
            )
            output_eps = os.path.join(
                OUTPUT_DIR,
                f"band_limited_pressure_u_{correlation_type}_{band}_3x1panel_AOA{AOA}.eps",
            )
