...
# CACHE_FILE = "/home/jofre/Members/Eduard/Paper2/Simulations/NACA_0012_AOA12_Re50000_1716x1662x128/Mean_data/Signal_correlation/3D_time_series/new/3D_time_series_AoA12_Re50000_high_freq_all_snapshots_20260610_190610.h5"
...
CACHE_FILE = "/home/jofre/Members/Eduard/Paper2/Simulations/NACA_0012_AOA5_Re50000_1716x1662x128/Mean_data/Signal_correlation/3D_time_series/new/3D_time_series_AoA5_Re50000_high_freq_all_snapshots_20260610_191508.h5" 
...
print("=" * 70)
print("3D HIGH-FREQUENCY BAND-LIMITED CORRELATION MAPS")
print("=" * 70)
...
with h5py.File(CACHE_FILE, 'r') as f:
    ...
    iteration_step = f.attrs.get('iteration_step', 800)
    ...
    # Validate Nyquist frequency for the high-frequency band.
    # The high band is defined as f > 12.5 Hz up to the resolved Nyquist frequency.
    if f_nyquist <= 12.5:
        raise RuntimeError(
            f"The high-frequency cache is not sufficiently resolved: "
            f"f_nyquist={f_nyquist:.6f} Hz <= 12.5 Hz. "
            "Use a cache with a smaller snapshot iteration step."
        )
    print(f"\n✓ High band is resolvable: 12.5 Hz < f <= {f_nyquist:.6f} Hz")
...
with h5py.File(CACHE_FILE, 'r') as f:
    ...
    for x_c_group in sorted(x_c_groups):
        ...
        # ====================================================================
        # Filtered RMS Diagnostics
        # ====================================================================

        print("\n  Filtered RMS diagnostics:")

        # With a single high-frequency band, the filtered signal is not expected
        # to reconstruct the full fluctuation field. Report the retained RMS
        # fraction instead of a reconstruction error.
        for signal_name, q_full in [
            ...
            for band_name in FREQUENCY_BANDS.keys():
                ...
                print(
                    f"    {signal_name:<10} / {band_name:<6}: "
                    f"RMS={q_band_rms:.6e}, ratio={q_band_rms / (q_full_rms + 1e-30):.6e}"
                )
        ...
        for band_name in FREQUENCY_BANDS.keys():
            ...
            print(
                f"    u_streamwise / {band_name:<6}: "
                f"RMS={u_band_rms:.6e}, ratio={u_band_rms / (u_full_rms + 1e-30):.6e}"
            )
        ...
        if not validation_passed:
            raise RuntimeError(f"Delta-z validation failed for {x_c_group}. Results were not saved.")

        # ====================================================================
        # Create Output Directories
        # ====================================================================

        case_fig_path = os.path.join(OUT_BASE_PATH, "figures_high", x_c_str)
...
output_file = os.path.join(
    OUT_BASE_PATH,
    "data",
    f"band_limited_3d_high_frequency_correlation_maps_{aoa_str}_Re50000.h5"
)
...
for x_c_actual in sorted(all_results.keys()):
    ...
    print(f"    Frequency bands: high={FREQUENCY_BANDS['high']}")
    x_c_str = result['x_c_str']
