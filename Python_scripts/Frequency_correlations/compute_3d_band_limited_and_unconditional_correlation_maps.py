...
# CACHE_FILE = "/home/jofre/Members/Eduard/Paper2/Simulations/NACA_0012_AOA5_Re50000_1716x1662x128/Mean_data/Signal_correlation/3D_time_series/new/3D_time_series_AoA5_Re50000_all_snapshots_20260604_222641.h5"
...
print("=" * 70)
print("3D BAND-LIMITED AND UNCONDITIONAL CORRELATION MAPS")
print("=" * 70)
...
def plot_correlation_maps(
    ...
    """
    Generate four plot types for correlation maps.

    Parameters
    ----------
    R : ndarray
        Correlation map with shape (N_valid_points, Nz).
    valid_x, valid_y : ndarray
        Spatial coordinates of valid points.
    Nz : int
        Number of z points.
    corr_abs_max : float
        Max absolute correlation value for color scaling.
    case_fig_path : str
        Path to save figures.
    corr_type : str
        Correlation type: "unconditional", "wall_only", or "both_filtered".
    signal_name : str
        Signal name: "tau_w" or "pressure".
    label_name : str
        Label for the correlation (e.g., "low", "mid", "full").
    x_c_str : str
        String representation of x/c location.
    """
    ...
    ax.set_title(
        f"R(Δz=0) - {signal_name} - {label_name}\n{corr_type}",
    ...
    fname = f"map_dz0_{corr_type}_{signal_name}_{label_name}_{x_c_str}.png"
    ...
    ax.set_ylabel("y/c", fontsize=11)
    ax.set_title(
        f"Max|R(Δz)| - {signal_name} - {label_name}\n{corr_type}",
        fontsize=12, fontweight="bold"
    )
    ax.set_aspect('equal', adjustable='box')
    cbar = plt.colorbar(scatter, ax=ax)
    ...
    fname = f"map_peak_abs_{corr_type}_{signal_name}_{label_name}_{x_c_str}.png"
    fpath = os.path.join(case_fig_path, fname)
    plt.savefig(fpath, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"      Saved: {fname}")
    ...
    ax.set_ylabel("y/c", fontsize=11)
    ax.set_title(
        f"Δz Index at Peak|R| - {signal_name} - {label_name}\n{corr_type}",
        fontsize=12, fontweight="bold"
    )
    ax.set_aspect('equal', adjustable='box')
    cbar = plt.colorbar(scatter, ax=ax)
    ...
    fname = f"map_peak_delta_z_index_{corr_type}_{signal_name}_{label_name}_{x_c_str}.png"
    fpath = os.path.join(case_fig_path, fname)
    plt.savefig(fpath, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"      Saved: {fname}")
    ...
    for raw_idx in selected_raw_dz_indices:
        ...
        ax.set_ylabel("y/c", fontsize=11)
        ax.set_title(
            f"R(raw_Δz={raw_idx}) - {signal_name} - {label_name}\n{corr_type}",
            fontsize=12, fontweight="bold"
        )
        ax.set_aspect('equal', adjustable='box')
        cbar = plt.colorbar(scatter, ax=ax)
        ...
        fname = f"map_dz_raw{raw_idx}_{corr_type}_{signal_name}_{label_name}_{x_c_str}.png"
        fpath = os.path.join(case_fig_path, fname)
        plt.savefig(fpath, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"      Saved: {fname}")
...
with h5py.File(CACHE_FILE, 'r') as f:
    ...
    for x_c_group in sorted(x_c_groups):
        ...
        # ====================================================================
        # Compute Band-Limited Correlations
        # ====================================================================

        print("\n  Computing band-limited 3D correlations:")
        ...
        # ====================================================================
        # Compute Unconditional Correlations
        # ====================================================================

        print("\n  Computing unconditional 3D correlations:")

        correlations_unconditional = {"tau_w": None, "pressure": None}

        # Unconditional: use full fluctuating signals without filtering
        correlations_unconditional["tau_w"] = compute_delta_z_correlation_fft_3d(
            tau_prime, u_prime, normalize=True
        )
        print(f"    tau_w (unconditional): shape={correlations_unconditional['tau_w'].shape}")

        correlations_unconditional["pressure"] = compute_delta_z_correlation_fft_3d(
            pressure_prime, u_prime, normalize=True
        )
        print(f"    pressure (unconditional): shape={correlations_unconditional['pressure'].shape}")

        # ====================================================================
        # Correlation Maxima (Band-Limited)
        # ====================================================================
        ...
        # ====================================================================
        # Unconditional Correlation Maxima
        # ====================================================================

        print("\n  Unconditional correlation maxima:")

        correlation_maxima["unconditional"] = {"tau_w": None, "pressure": None}

        for signal_name in ["tau_w", "pressure"]:
            R = correlations_unconditional[signal_name]
            R_min = np.nanmin(R)
            R_max = np.nanmax(R)
            R_abs_max = np.nanmax(np.abs(R))
            correlation_maxima["unconditional"][signal_name] = R_abs_max
            print(
                f"    unconditional / {signal_name:<8}: "
                f"min R={R_min:.6e}, max R={R_max:.6e}, max|R|={R_abs_max:.6e}"
            )

        # ====================================================================
        # Validation: Delta z = 0 Consistency (Band-Limited)
        # ====================================================================
        ...
        # ====================================================================
        # Validation: Delta z = 0 Consistency (Unconditional)
        # ====================================================================
        ...
        for signal_name in ["tau_w", "pressure"]:
            q_full = tau_prime if signal_name == "tau_w" else pressure_prime
            q_rms = np.sqrt(np.mean(q_full**2))

            R_dz0_uncond = correlations_unconditional[signal_name][:, 0]
            u_rms = np.sqrt(np.mean(u_prime**2, axis=(0, 2)))
            R_scalar_uncond = np.mean(q_full[:, None, :] * u_prime, axis=(0, 2)) / (q_rms * u_rms + 1e-30)

            error_uncond = np.max(np.abs(R_dz0_uncond - R_scalar_uncond))

            if error_uncond > 1e-10:
                print(f"    ✗ {signal_name} (unconditional): error={error_uncond:.6e}")
                validation_passed = False
            else:
                print(f"    ✓ {signal_name} (unconditional): error={error_uncond:.6e}")
        ...
        # ====================================================================
        # Create Scatter Plots (Band-Limited)
        # ====================================================================

        print("\n  Creating band-limited scatter plots:")
        ...
        # ====================================================================
        # Create Scatter Plots (Unconditional)
        # ====================================================================

        print("\n  Creating unconditional scatter plots:")

        for signal_name in ["tau_w", "pressure"]:
            R = correlations_unconditional[signal_name]
            corr_abs_max = correlation_maxima["unconditional"][signal_name]
        ...
        all_results[x_c_actual] = {
            ...
            'correlations_unconditional': correlations_unconditional,
...
output_file = os.path.join(
    ...
    f"band_limited_and_unconditional_3d_correlation_maps_{aoa_str}_Re50000.h5"
)
...
with h5py.File(output_file, 'w') as hf:
    ...
    meta_grp.attrs['contains_unconditional_correlations'] = True
    meta_grp.attrs['unconditional_definition'] = (
        "Correlation computed from full fluctuating wall signal and full "
        "fluctuating streamwise velocity field, without temporal filtering."
    )
    ...
    for x_c_actual in sorted(all_results.keys()):
        ...
        uncond_grp = loc_grp.create_group('unconditional')
        for signal_name in ["tau_w", "pressure"]:
            sig_grp = uncond_grp.create_group(signal_name)
            R = result['correlations_unconditional'][signal_name]
            ds = sig_grp.create_dataset('R', data=R, dtype=np.float64)
            ds.attrs['plot_abs_max'] = result['correlation_maxima']['unconditional'][signal_name]
...
for x_c_actual in sorted(all_results.keys()):
    ...
    print(f"      - unconditional R(tau'_w, u')")
    print(f"      - unconditional R(p'_w, u')")
    print(f"      - band-limited wall-only correlations")
    print(f"      - band-limited both-filtered correlations")
    x_c_str = result['x_c_str']
