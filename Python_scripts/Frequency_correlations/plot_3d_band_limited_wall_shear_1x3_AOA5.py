...
def colorbar_title_for_band(band):
    ...
    if band == "low":
        return r"$R_{\tau_{w,L}^{\prime}u^{\prime}}$"
    if band == "mid":
        return r"$R_{\tau_{w,M}^{\prime}u^{\prime}}$"
    return r"$R_{\tau_{w,B}^{\prime}u^{\prime}}$"
...
def main():
    ...
    for correlation_type in CORRELATION_TYPES:
        for band in BANDS:
            ...
            # Positive-only plotting convention, same as your current colormap.
            vmin_plot = VMIN
            ...
            output_png = os.path.join(
                OUTPUT_DIR,
                f"band_limited_tau_w_u_{correlation_type}_{band}_1x3panel_AOA{AOA}.png",
            )
            output_eps = os.path.join(
                OUTPUT_DIR,
                f"band_limited_tau_w_u_{correlation_type}_{band}_1x3panel_AOA{AOA}.eps",
            )
