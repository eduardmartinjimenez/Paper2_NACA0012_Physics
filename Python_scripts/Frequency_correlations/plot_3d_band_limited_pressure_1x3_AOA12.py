"""
Band-limited wall-pressure / streamwise-velocity correlation maps, AoA 12.
...
unconditional wall-shear correlation 1x3 panel figure, but uses the frequency-
filtered correlation fields from:

    R_{p_{w,B}' u_s'}(x,y, Delta z=0)

Two correlation variants are plotted here:

    /x_c_*/wall_only/pressure/{low,mid}_R
    /x_c_*/both_filtered/pressure/{low,mid}_R
...
CORR_FILE = (
    ...
    "Freq_correlations_3D/band_limited_3d_correlation_maps_AOA12_Re50000.h5"
)
...
TS_FILE = (
    "/home/jofre/Members/Eduard/Paper2/Simulations/"
    "NACA_0012_AOA12_Re50000_1716x1662x128/Mean_data/"
    "Signal_correlation/3D_time_series/"
    "3D_time_series_AoA12_Re50000_all_snapshots_20260514_022417.h5"
)
...
# Color scale lower bound. The upper bound is computed separately for each
# frequency band from the loaded correlations.
VMIN = 0.0
N_LEVELS = 101

# If the filtered correlations contain weak negative values, clip them to the
# positive plotting range. Set to False to inspect signed values.
...
def add_airfoil_overlay(ax, airfoil, x_ref, y_ref):
    ...
    ax.plot(x_upper_sorted, y_upper_sorted, "k-", linewidth=1.0, zorder=12)
    ax.plot(x_lower_sorted, y_lower_sorted, "k-", linewidth=1.0, zorder=12)
...
def plot_tricontour_or_scatter(ax, x, y, R, levels_r, vmin, vmax, reference_contour):
    ...
    try:
        ...
        # Add reference contour only if the level is within the data range.
        if np.nanmin(R) <= reference_contour <= np.nanmax(R):
            ax.tricontour(
                tri,
                R,
...
def colorbar_title_for_band(band):
    # Keep compact notation. L and M denote low and mid band.
    if band == "low":
        return r"$R_{p_{w,L}^{\prime}u^{\prime}}$"
    if band == "mid":
        return r"$R_{p_{w,M}^{\prime}u^{\prime}}$"
    return r"$R_{p_{w,B}^{\prime}u^{\prime}}$"

def get_visible_window_values(panel_data):
    """
    Collect R values only from the points inside the plotted visualization window
    for all panels in the current figure.
    """
    values = []
    ...
    if not values:
        raise RuntimeError(
            "No valid correlation points found inside the plotted windows. "
...
def main():
    print("=" * 70)
    print("BAND-LIMITED WALL-PRESSURE CORRELATION MAPS")
    print("=" * 70)
    ...
    for correlation_type in CORRELATION_TYPES:
        for band in BANDS:
            print("\n" + "=" * 70)
            print(f"CREATING 1x3 PANEL FOR TYPE: {correlation_type}, BAND: {band}")
            print("=" * 70)
            ...
            band_vmin = float(np.nanmin(visible_values))
            band_vmax = float(np.nanmax(visible_values))

            if CLIP_TO_COLOR_RANGE:
                # Positive-only plotting style, same as the old unconditional plots.
                vmin_plot = VMIN
                vmax_plot = max(band_vmax, VMIN + 1e-12)
            else:
                # Signed plotting style.
                vmax_abs = max(abs(band_vmin), abs(band_vmax))
                vmin_plot = -vmax_abs
                vmax_plot = vmax_abs
            ...
            print(f"\n  {correlation_type} / {band} plot limits from visible window:")
            print(f"    visible raw range: [{band_vmin:.4e}, {band_vmax:.4e}]")
            ...
            for col, data in enumerate(panel_data):
                ...
                ax.set_title(rf"$x/c = {X_C_LOCATIONS[col]:.2f}$", fontsize=12)
                ...
                print(f"\nPanel {col + 1} ({correlation_type}, {band}, x/c={X_C_LOCATIONS[col]:.2f}):")
            ...
            cbar.ax.set_title(colorbar_title_for_band(band), fontsize=14, pad=10)

            fig.suptitle(
                f"Band-limited wall-pressure/velocity correlation, {band_label_for_title(band)}, {correlation_type_label(correlation_type)}, AoA={AOA}°",
                fontsize=14,
                fontweight="bold",
            )
            ...
            output_png = os.path.join(
                OUTPUT_DIR,
                f"band_limited_pressure_u_{correlation_type}_{band}_1x3panel_AOA{AOA}.png",
            )
            output_eps = os.path.join(
                OUTPUT_DIR,
                f"band_limited_pressure_u_{correlation_type}_{band}_1x3panel_AOA{AOA}.eps",
            )
