...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "probe_time_signals.h5")
...
def cross_correlation_fft(a, b):
    """
    Linear (zero-padded) cross-correlation of equal-length 1-D arrays a and b.

    Returns
    -------
    lags  : int array  [-N+1, ..., 0, ..., N-1]
    R_tau : float array normalised by N * std(a) * std(b)
    running_R0 : cumulative zero-lag correlation (convergence check)
    """
    N = len(a)
    a = a - np.mean(a)
    b = b - np.mean(b)
    ...
    if sigma_a < 1e-14 or sigma_b < 1e-14:
        lags      = np.arange(-(N - 1), N)
        R_tau     = np.zeros(2 * N - 1)
        running_R0 = np.zeros(N)
        return lags, R_tau, running_R0

    a_norm = a / sigma_a
    b_norm = b / sigma_b

    n_fft  = 2 * N - 1
    A_fft  = np.fft.rfft(a_norm, n=n_fft)
    B_fft  = np.fft.rfft(b_norm, n=n_fft)
    R_full = np.fft.irfft(np.conj(A_fft) * B_fft, n=n_fft)[:n_fft] / N

    lags  = np.arange(-(N - 1), N)
    R_tau = np.concatenate([R_full[N:], R_full[:N]])

    running_R0 = np.cumsum(a_norm * b_norm) / np.arange(1, N + 1)
...
# ============================================================================
print("=" * 70)
print("PROBE TIME-SIGNAL EXTRACTION AND CORRELATION")
print("=" * 70)
...
# ============================================================================
# STEP 7 — Compute zero-lag correlations (FFT Method)
# ============================================================================
print("\n" + "=" * 70)
print("STEP 7  —  COMPUTING ZERO-LAG CORRELATIONS (FFT Method)")
print("=" * 70)

corr_results = {}   # key: (k, j)

for k, sinfo in enumerate(surf_probe_info):
    for j, dinfo in enumerate(domain_probe_info):
        lags, R_tau, running_R0 = cross_correlation_fft(
            tau_prime_ts[k], u_prime_ts[j]
        )
        # Extract zero-lag correlation (lag=0 is at index n_valid-1)
        zero_lag_idx = n_valid - 1
        R0 = float(R_tau[zero_lag_idx])
        ...
        print(f"  surf[{k}] (x/c={sinfo['xc_actual']:.3f})  x  "
              f"dom[{j}] ({dinfo['x_actual']:.3f}, {dinfo['y_actual']:.3f})"
              f"  ->  R(0)={R0:.4f}")
...
with h5py.File(OUTPUT_FILE, "w") as hf:
    ...
    hf.attrs['correlation_method'] = 'fft'
...
fig.suptitle('Zero-lag correlation convergence (FFT Method)', fontsize=13, fontweight='bold')
plt.tight_layout()
plt.show()
