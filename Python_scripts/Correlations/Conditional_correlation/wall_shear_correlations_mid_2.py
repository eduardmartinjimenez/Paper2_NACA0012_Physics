...
# ============================================================================
# Timestamp matching: ensure surface and snapshot files correspond
# ============================================================================
print("\n" + "=" * 70)
print("MATCHING SURFACE AND SNAPSHOT FILES BY TIMESTAMP")
print("=" * 70)

def extract_timestamp(filename):
    """Extract the iteration number (timestamp) from a filename.

    Examples:
        surface_3d_NACA0012_Re50000_AoA12_6350000-COMP-DATA.h5 -> '6350000'
        3d_NACA0012_Re50000_AoA12_6350000-COMP-DATA.h5          -> '6350000'
    """
...
# Build dictionaries keyed by timestamp (detect duplicates)
surface_by_ts = {}
surface_duplicates = []
for f in all_surface_files:
    ts = extract_timestamp(f)
    if ts in surface_by_ts:
        surface_duplicates.append((ts, surface_by_ts[ts], f))
    surface_by_ts[ts] = f
...
for f in all_snapshots_files:
    ...
    if ts in snapshot_by_ts:
        snapshot_duplicates.append((ts, snapshot_by_ts[ts], f))
    snapshot_by_ts[ts] = f

# Report duplicates
if surface_duplicates:
    print(f"  [WARNING] {len(surface_duplicates)} duplicate timestamp(s) in surface files:")
    for ts, f1, f2 in surface_duplicates:
        print(f"    timestamp {ts}:")
        print(f"      {os.path.basename(f1)}")
        print(f"      {os.path.basename(f2)}  (kept)")
if snapshot_duplicates:
    print(f"  [WARNING] {len(snapshot_duplicates)} duplicate timestamp(s) in snapshot files:")
    for ts, f1, f2 in snapshot_duplicates:
        print(f"    timestamp {ts}:")
        print(f"      {os.path.basename(f1)}")
        print(f"      {os.path.basename(f2)}  (kept)")

# Find common timestamps (sorted)
common_timestamps = sorted(set(surface_by_ts.keys()) & set(snapshot_by_ts.keys()), key=int)

print(f"  Surface files:  {len(all_surface_files)} ({len(surface_by_ts)} unique timestamps)")
print(f"  Snapshot files: {len(all_snapshots_files)} ({len(snapshot_by_ts)} unique timestamps)")
print(f"  Matched pairs:  {len(common_timestamps)}")

if len(common_timestamps) == 0:
    raise RuntimeError("No matching timestamps found between surface and snapshot files!")

# Report unmatched timestamps
unmatched_surf_ts = sorted(set(surface_by_ts.keys()) - set(snapshot_by_ts.keys()), key=int)
unmatched_snap_ts = sorted(set(snapshot_by_ts.keys()) - set(surface_by_ts.keys()), key=int)

if unmatched_surf_ts:
    print(f"  [WARNING] {len(unmatched_surf_ts)} surface files have no matching snapshot:")
    for ts in unmatched_surf_ts:
        print(f"    timestamp {ts}: {os.path.basename(surface_by_ts[ts])}")
if unmatched_snap_ts:
    print(f"  [WARNING] {len(unmatched_snap_ts)} snapshot files have no matching surface file:")
    for ts in unmatched_snap_ts:
        print(f"    timestamp {ts}: {os.path.basename(snapshot_by_ts[ts])}")

# Build matched lists (sorted by timestamp)
...
print(f"  First timestamp: {common_timestamps[0]}")
print(f"  Last timestamp:  {common_timestamps[-1]}")
...
N_matched = len(matched_surface_files)
print(f"Loading {N_matched} matched surface snapshots to compute mean...")

for idx, surface_file in enumerate(matched_surface_files):
    if (idx + 1) % 20 == 0 or idx == 0:
        print(f"  Loading surface snapshot {idx+1}/{N_matched}...", flush=True)
...
# Get middle z-plane index (used for point-based conditioning)
z_mid = Nz_phys // 2
print(f"  Middle z-plane index: z_mid = {z_mid}")
...
for x_c_target in point_indices.keys():
    surface_data[x_c_target] = {
        'tau_prime_w': []  # One scalar (z_mid value) per snapshot
    }

print(f"Processing {n_snapshots} matched snapshots for surface data (z_mid only)...")

for idx, surface_file in enumerate(matched_surface_files[:n_snapshots]):
    if (idx + 1) % 20 == 0 or idx == 0:
    ...
    try:
        ...
        # Extract at each chord location — only z_mid plane
        for x_c_target, point_info in point_indices.items():
            idx_point = point_info['indices'][0]  # Single point

            # Extract surface value at z_mid for this point
            tau_at_xc_zmid = tau_w[z_mid, idx_point]  # scalar

            # Compute fluctuation
            tau_prime_zmid = tau_at_xc_zmid - tau_w_mean[idx_point]
            surface_data[x_c_target]['tau_prime_w'].append(tau_prime_zmid)  # one scalar per snapshot
    except Exception as e:
...
print(f"  Cropped array shape: (Ny={Ny_crop}, Nx={Nx_crop})")
print(f"  Correlation will use z_mid plane only")
...
avg_u_data = avg_u_data[iy_min:iy_max, ix_min:ix_max]  # (Ny_crop, Nx_crop)
avg_v_data = avg_v_data[iy_min:iy_max, ix_min:ix_max]  # (Ny_crop, Nx_crop)
...
# - Numerator_PF, Numerator_NF, Numerator_all (2D: z_mid plane only)
...
for x_c_target in point_indices.keys():
    correlation_data[x_c_target] = {
        'Numerator_PF': np.zeros((Ny_crop, Nx_crop), dtype=np.float64),
        'Numerator_NF': np.zeros((Ny_crop, Nx_crop), dtype=np.float64),
        'Numerator_all': np.zeros((Ny_crop, Nx_crop), dtype=np.float64),
        'u_prime_sq': np.zeros((Ny_crop, Nx_crop), dtype=np.float64),
        'N_PF': 0,
        'N_NF': 0,
        'N_all': 0,
...
# We'll process matched snapshots and accumulate correlations
print(f"Processing {n_snapshots} matched snapshot pairs...")

# Pre-compute cropped mean field (2D)
avg_u_crop = avg_u_data[iy_min:iy_max, ix_min:ix_max]  # (Ny_crop, Nx_crop)

# Set up cropped 2D reconstruction (z_mid plane, cropped y/x window)
loader.set_crop_region(z_mid, (iy_min, iy_max), (ix_min, ix_max))
...
for snap_idx in range(n_snapshots):
    ...
    snapshot_file = matched_snapshot_files[snap_idx]

    try:
        # Load instantaneous velocity fields (full compressed)
        fields_inst = loader.load_snapshot(snapshot_file)

        # Reconstruct full 3D fields, extract z_mid plane, crop, then delete full arrays
        u_full = loader.reconstruct_field(fields_inst["u"])
        u_cropp = u_full[1:-1, iy_min:iy_max, ix_min:ix_max]  # (Ny_crop, Nx_crop)
        del u_full  # Free memory

        v_full = loader.reconstruct_field(fields_inst["v"])
        v_cropp = v_full[1:-1, iy_min:iy_max, ix_min:ix_max]  # (Ny_crop, Nx_crop)
        del v_full  # Free memory

        # Compute streamwise velocity at z_mid (aligned with angle of attack)
        u_inst = u_cropp * np.cos(AOA_rad) + v_cropp * np.sin(AOA_rad)  # (Ny_crop, Nx_crop)

        # Compute fluctuation: u' = u - <u>
        u_prime = u_inst - avg_u_crop  # (Ny_crop, Nx_crop)

        # Replace NaN values with 0 (inside airfoil)
        u_prime = np.where(np.isnan(u_prime), 0.0, u_prime)
        ...
        for x_c_target, point_info in point_indices.items():
            ...
            # Retrieve z_mid tau'_w for this snapshot (single scalar)
            tau_prime_val = surface_data[x_c_target]['tau_prime_w'][snap_idx]
            ...
            # Correlate scalar tau'_w(z_mid) with 2D cropped velocity field
            product = tau_prime_val * u_prime  # (Ny_crop, Nx_crop)

            # Accumulate correlation numerator
            correlation_data[x_c_target]['Numerator_all'] += product
            correlation_data[x_c_target]['N_all'] += 1

            # Accumulate u'^2 for RMS (only from classified samples)
...
for x_c_target, corr_data in correlation_data.items():
    ...
    # Compute RMS of u' at each spatial location
    ...
    if N_PF > 0:
        R_PF = Numerator_PF / (N_PF * tau_rms_PF * u_rms_safe)
    else:
        R_PF = np.zeros_like(R_all)

    if N_NF > 0:
        R_NF = Numerator_NF / (N_NF * tau_rms_NF * u_rms_safe)
    else:
...
for x_c_target, results in correlation_results.items():
    ...
    output_filename = f"wall_shear_correlation_xc_{x_c_target:.3f}_alpha_{ALPHA:.1f}_200_2.h5"
    ...
    with h5py.File(output_path, "w") as f:
        ...
        # Correlation fields (2D: z_mid plane)
        ...
        # Grid coordinates (cropped, z_mid plane only — 2D)
        x_crop = x_data[z_mid, iy_min:iy_max, ix_min:ix_max]
        y_crop = y_data[z_mid, iy_min:iy_max, ix_min:ix_max]

        f.create_dataset('x', data=x_crop, compression='gzip')
        f.create_dataset('y', data=y_crop, compression='gzip')
        f.attrs['z_mid_index'] = z_mid
