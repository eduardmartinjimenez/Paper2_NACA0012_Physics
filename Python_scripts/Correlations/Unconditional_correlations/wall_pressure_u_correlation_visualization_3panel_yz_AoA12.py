"""
Wall-Pressure & Streamwise Velocity Correlation Visualization Script (3x1 Panel, Y-Z Plane)

This script generates 3-panel visualizations of unconditional correlations between
...
- Centered spanwise separation (Δz=0 at middle for symmetric correlation decay)
...
VIZ_YLIM_OFFSET = [0.012, 0.2]  # y-window (wall-normal direction)
...
levels_r = np.linspace(r_global_min, r_global_max, 101)
...
for col, data in enumerate(data_list):
    ...
    ax.set_xlabel(r'$\Delta z / c$', fontsize=11)
...
# Save figure
output_path_png = os.path.join(OUTPUT_DIR, "wall_pressure_u_correlation_1x3panel_yz_AOA12.png")
output_path_eps = os.path.join(OUTPUT_DIR, "wall_pressure_u_correlation_1x3panel_yz_AOA12.eps")
