...
wall-pressure (p_w) and streamwise velocity (u') fluctuations at three chord
locations (x/c = 0.5, 0.7, 0.9).

Features:
- AOA-based coordinate rotation (aligns field with local flow direction)
- Correlation range fixed to [-1, 1]
- Dynamic visualization window centered on each location
- 3x1 vertical layout with signed diverging colormap
"""
...
cbar.set_label(r'$R_{p_w^\prime u^\prime}$', fontsize=14)

# Save figure
output_path_png = os.path.join(OUTPUT_DIR, "wall_pressure_u_correlation_3x1panel_AOA12.png")
output_path_eps = os.path.join(OUTPUT_DIR, "wall_pressure_u_correlation_3x1panel_AOA12.eps")
...
print(f"  Correlation type: p_w vs u' fluctuations")
