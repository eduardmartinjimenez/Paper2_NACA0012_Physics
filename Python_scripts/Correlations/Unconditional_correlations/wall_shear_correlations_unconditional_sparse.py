...
# Output directory
OUTPUT_DIR = "/home/jofre/Members/Eduard/Paper2/Simulations/NACA_0012_AOA5_Re50000_1716x1662x128/Mean_data/Wall_shear_correlations/test_3_sparse/"
...
for x_c_target, point_info in point_indices.items():
    ...
    reduction_factor = (Nx_full_crop * Ny_full_crop) / (Nx_sparse * Ny_sparse)
    print(f"    Approximate x-y memory reduction factor: {reduction_factor:.2f}x")
...
ax.scatter(x_sparse, y_sparse, s=2, c='black', alpha=0.3, label='Subsampled velocity points')
...
ax.set_title(f'Correlation Domain for x/c = {x_c_target:.2f}\n(Spatially Subsampled Velocity Points)', fontsize=16, fontweight='bold')
...
print("USING SPATIALLY SUBSAMPLED VELOCITY POINTS (X-Y)")
print("=" * 70)
...
for snap_idx in range(n_snapshots):
    ...
    try:
        ...
        for x_c_target, point_info in point_indices.items():
            ...
            stride_x = crop_info['stride_x']
            stride_y = crop_info['stride_y']

            # Crop to correlation window with spatial subsampling
            u_inst = u_inst_full[1:-1,
                                 iy_min:iy_max:stride_y,
                                 ix_min:ix_max:stride_x]  # (Nz_phys, Ny_sparse, Nx_sparse)

            # Compute fluctuation: u' = u - <u>
            avg_u_crop = avg_u_data[iy_min:iy_max:stride_y,
                                    ix_min:ix_max:stride_x]  # (Ny_sparse, Nx_sparse)
            u_prime = u_inst - avg_u_crop[np.newaxis, :, :]  # (Nz_phys, Ny_sparse, Nx_sparse)

            # Verify shape matches accumulator
            if u_prime.shape != correlation_data[x_c_target]['numerator'].shape:
                raise RuntimeError(
                    f"Shape mismatch for x/c={x_c_target}: "
                    f"u_prime shape {u_prime.shape}, "
                    f"accumulator shape {correlation_data[x_c_target]['numerator'].shape}"
                )
            ...
            u_fft = np.fft.rfft(u_prime, axis=0)  # (Nz//2+1, Ny_sparse, Nx_sparse)
            u2_fft = np.fft.rfft(u_prime**2, axis=0)  # (Nz//2+1, Ny_sparse, Nx_sparse)
            ...
            num = np.fft.irfft(np.conj(tau_fft[:, None, None]) * u_fft,
                                n=Nz_phys, axis=0)  # (Nz_phys, Ny_sparse, Nx_sparse)
...
for x_c_target, corr_data in correlation_data.items():
    ...
    print(f"    u_rms shape: {u_rms.shape}")
...
for x_c_target, results in correlation_results.items():
    ...
    stride_x = crop_info['stride_x']
    stride_y = crop_info['stride_y']
    stride_z = crop_info['stride_z']

    output_filename = (
        f"wall_shear_correlation_unconditional_sparse_"
        f"xc_{x_c_target:.3f}_"
        f"sx{STRIDE_X}_sy{STRIDE_Y}_sz{STRIDE_Z}.h5"
    )
    ...
    with h5py.File(output_path, "w") as f:
        ...
        f.attrs['subsampled_xy'] = True
        f.attrs['full_spanwise_domain'] = True

        # Crop size metadata
        f.attrs['Nx_full_crop'] = ix_max - ix_min
        f.attrs['Ny_full_crop'] = iy_max - iy_min
        f.attrs['Nx_sparse_crop'] = results['r'].shape[2]
        f.attrs['Ny_sparse_crop'] = results['r'].shape[1]
        f.attrs['Nz_crop'] = results['r'].shape[0]

        # Correlation field
        f.create_dataset('R', data=results['r'], compression='gzip')
        f.create_dataset('u_rms', data=results['u_rms'], compression='gzip')

        # Grid coordinates (cropped and subsampled for this x/c location, all z-planes)
        x_crop = x_data[:, iy_min:iy_max:stride_y, ix_min:ix_max:stride_x]
        y_crop = y_data[:, iy_min:iy_max:stride_y, ix_min:ix_max:stride_x]
        z_crop = z_data[:, iy_min:iy_max:stride_y, ix_min:ix_max:stride_x]

        # Verify coordinate shapes match results
        assert x_crop.shape == results['r'].shape, f"x_crop shape {x_crop.shape} != r shape {results['r'].shape}"
        assert y_crop.shape == results['r'].shape, f"y_crop shape {y_crop.shape} != r shape {results['r'].shape}"
        assert z_crop.shape == results['r'].shape, f"z_crop shape {z_crop.shape} != r shape {results['r'].shape}"
...
print(f"Output format: wall_shear_correlation_unconditional_sparse_xc_X.XXX_sxS_syS_szS.h5")
