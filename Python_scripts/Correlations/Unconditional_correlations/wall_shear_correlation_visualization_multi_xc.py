"""
Wall-Shear Correlation Visualization Script (Multi x/c Locations)

This script generates multi-location comparison visualizations of unconditional
wall-shear stress correlations from CFD simulations. It scans a results directory
...
h5_files = sorted(glob.glob(os.path.join(RESULTS_DIR, "wall_shear_correlation_unconditional_xc_*.h5")))
...
fig.suptitle(
    r'Multi-location comparison of $u^\prime_{rms}/U_\infty$ and correlation at $\Delta z = 0$',
    fontsize=14
)

# Save figure
fig4b_path = os.path.join(OUTPUT_DIR, "FIGURE_4B_multi_location_u_rms_correlation.png")
fig.savefig(fig4b_path, dpi=150, bbox_inches='tight')
print(f"\nFIGURE 4B saved: {fig4b_path}")
...
fig.suptitle('Multi-location comparison of correlation fields at different spanwise separations', fontsize=14)

# Save figure
fig5b_path = os.path.join(OUTPUT_DIR, "FIGURE_5B_multi_location_spanwise_variation.png")
fig.savefig(fig5b_path, dpi=150, bbox_inches='tight')
print(f"FIGURE 5B saved: {fig5b_path}")
...
print(f"\nOUTPUT FIGURES:")
print(f"  FIGURE 4B: {fig4b_path}")
print(f"  FIGURE 5B: {fig5b_path}")
