"""
Spanwise correlation decay profiles of unconditional wall-pressure/velocity correlation.
...
1. Loads wall_pressure_correlation_unconditional_xc_*.h5 (full 3D field).
2. Extracts the spanwise correlation curve at the surface point.
3. Recenters the lag coordinate so Δz=0 is in the middle.
4. Creates a single comparison plot showing all 6 correlations (3 x/c × 2 AoAs).
...
def load_full_correlation(filepath):
    ...
    with h5py.File(filepath, "r") as f:
        ...
        p_rms = float(f.attrs.get("p_total_rms", np.nan))
        p_mean = float(f.attrs.get("p_total_mean", np.nan))
...
print("=" * 78)
print("UNCONDITIONAL WALL-PRESSURE / STREAMWISE-VELOCITY SPANWISE CORRELATION DECAY")
print("=" * 78)
...
ax.axhline(0.0, color="0.4", linewidth=0.8, alpha=0.6)
ax.axvline(0.0, color="green", linestyle="--", linewidth=1.5, alpha=0.5)

ax.set_xlabel(r"$\Delta z / c$", fontsize=12)
ax.set_ylabel(r"$R_{p^\prime u^\prime}$", fontsize=12)
...
aoa_handles = [
    Line2D([0], [0], color="black", linewidth=2.5, linestyle=linestyle_by_case[case_name],
           label=f"AoA = {CASES[case_name]['AOA_DEG']:.0f}°")
    for case_name in sorted(profiles.keys())
]

ax.legend(handles=color_handles + aoa_handles, fontsize=11, loc="best", frameon=False, ncol=2)

fig.tight_layout()

plot_png = os.path.join(
    OUTPUT_DIR,
    "R_p_u_unconditional_spanwise_decay_AOA5_AOA12.png",
)
