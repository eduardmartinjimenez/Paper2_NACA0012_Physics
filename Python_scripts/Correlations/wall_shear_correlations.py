...
for idx, surface_file in enumerate(all_surface_files[:n_snapshots]):
    ...
    try:
        ...
        for x_c_target, point_info in point_indices.items():
            ...
            surface_data[x_c_target]['tau_prime_w'].extend(tau_at_xc_fluct)
...
sample_global_idx = 0  # Global sample counter across all snapshots and z-planes
...
for snap_idx in range(n_snapshots):
    ...
    try:
        ...
        u_prime = u_inst - avg_u_crop[np.newaxis, :, :]  # Broadcasting over z
        ...
        for x_c_target, point_info in point_indices.items():
            ...
            # Loop over each z-plane (reference location)
            for z_k in range(Nz_phys):
                tau_prime_val = tau_prime_current[z_k]
                ...
                # Roll u_prime so that z_k -> Δz=0 (periodic in z)
                u_prime_rolled = np.roll(u_prime, -z_k, axis=0)

                # Accumulate correlation numerator: tau'_w * u'
                product = tau_prime_val * u_prime_rolled

                correlation_data[x_c_target]['Numerator_all'] += product
                correlation_data[x_c_target]['N_all'] += 1

                # Accumulate u'^2 for RMS (same for all samples)
                correlation_data[x_c_target]['u_prime_sq'] += u_prime_rolled**2
...
for x_c_target, corr_data in correlation_data.items():
    ...
    # R = <tau'_w * u'> / (tau_rms * u_rms)
    # Conditional: R_PF = Numerator_PF / (N_PF * tau_rms_PF * u_rms)

    # Avoid division by zero in u_rms
    u_rms_safe = np.where(u_rms > 1e-12, u_rms, 1e-12)

    # Unconditioned correlation
    R_all = corr_data['Numerator_all'] / (N_all * tau_rms_dict[x_c_target] * u_rms_safe)

    # Conditional correlations
    if N_PF > 0:
        R_PF = corr_data['Numerator_PF'] / (N_PF * tau_rms_PF * u_rms_safe)
    else:
        R_PF = np.zeros_like(R_all)

    if N_NF > 0:
        R_NF = corr_data['Numerator_NF'] / (N_NF * tau_rms_NF * u_rms_safe)
    else:
    ...
    if ALPHA == 0.0:
        Num_sum = corr_data['Numerator_PF'] + corr_data['Numerator_NF']
        Num_diff = np.max(np.abs(corr_data['Numerator_all'] - Num_sum))
...
for x_c_target, results in correlation_results.items():
    ...
    output_filename = f"wall_shear_correlation_xc_{x_c_target:.3f}.h5"
