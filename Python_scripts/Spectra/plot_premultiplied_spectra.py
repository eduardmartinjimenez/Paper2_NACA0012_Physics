...
def load_spectra_data(data_dir, aoa_label):
    ...
    data_files = sorted(
        ...
        if f.endswith(".h5") and "premultiplied_spectra_data_all_final" in f
    )
...
#     contour_fill = ax.contourf(X, Y, Z, levels=local_levels, cmap='RdYlBu_r', extend='neither')
...
def add_compact_colorbar(fig, ax, contour_fill):
    """Add compact vertical colorbar with label placed on top."""
    cbar = fig.colorbar(
        contour_fill,
        ax=ax,
        pad=0.075,
...
for col, data in enumerate(datasets_aoa5_selected):
    ...
    cbar = add_compact_colorbar(fig, ax, contour_fill)
...
plt.tight_layout()
os.makedirs(OUTPUT_PATH, exist_ok=True)
fig.savefig(os.path.join(OUTPUT_PATH, "premultiplied_spectra_2x3.eps"), dpi=300, bbox_inches="tight")
fig.savefig(os.path.join(OUTPUT_PATH, "premultiplied_spectra_2x3.png"), dpi=300, bbox_inches="tight")
plt.show()
