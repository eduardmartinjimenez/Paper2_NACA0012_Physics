...
if cache_metadata is not None:
    ...
    if is_valid:
        ...
        try:
            ...
            # Recompute fluctuations from raw cached signals.
            # Arrays have shape (Nt, Nz), so axis=0 is the time direction.
            timeseries_data['tau_w_prime'] = (
                timeseries_data['tau_w_z']
                - np.mean(timeseries_data['tau_w_z'], axis=0, keepdims=True)
            )

            timeseries_data['pressure_prime'] = (
                timeseries_data['pressure_z']
                - np.mean(timeseries_data['pressure_z'], axis=0, keepdims=True)
            )

            for i in range(len(probe_info)):
                timeseries_data[f'u_s_prime_{i}'] = (
                    timeseries_data[f'u_s_z_{i}']
                    - np.mean(timeseries_data[f'u_s_z_{i}'], axis=0, keepdims=True)
                )

            print("✓ Loaded raw time series from cache and recomputed fluctuations")
        except Exception as e:
...
if timeseries_data is None:
    ...
    # Compute fluctuations by subtracting the temporal mean at each spanwise plane.
    # Arrays have shape (Nt, Nz), so axis=0 is time.
    timeseries_data['tau_w_prime'] = (
        timeseries_data['tau_w_z']
        - np.mean(timeseries_data['tau_w_z'], axis=0, keepdims=True)
    )
