"""
Multi-Probe Spectral Analysis - Full Spanwise Domain (Pressure)
================================================================
...
3. Extracts signals p'(t,z) and u'_i(t,z) for all z-positions across all snapshots
...
# # Cache paths
# # TIMESERIES_CACHE_PATH = "/home/jofre/Members/Eduard/Paper2/Simulations/NACA_0012_AOA12_Re50000_1716x1662x128/Mean_data/Pressure_correlations/test_3/timeseries_spanwise_cache/"
...
# Cache paths
# TIMESERIES_CACHE_PATH = "/home/jofre/Members/Eduard/Paper2/Simulations/NACA_0012_AOA5_Re50000_1716x1662x128/Mean_data/Wall_pressure_correlations/test_2/timeseries_spanwise_cache/"
TIMESERIES_CACHE_PATH = None
...
def compute_pressure_all_z(p_data, y_idx):
    ...
    """
    # Extract pressure at surface y_idx for ALL z (at x=0, the first point)
    p = p_data[:, y_idx, 0]
...
cbar = plt.colorbar(im, ax=ax, label=r'$R_{p^\prime u^\prime}$')
...
ax.set_title(r'Spatial $R_{p^\prime u^\prime}$ Correlation with Airfoil Geometry and Probes ($\Delta z = 0$)',
...
if timeseries_data is None:
    ...
    timeseries_data = {
        'iterations': [],
        'ref_point': {'p_z': []},  # Will be (Nt, Nz) - raw pressure at surface
    }
    ...
    for file_idx, data_file in enumerate(data_files):
        ...
        try:
            ...
            # Extract pressure for surface point across all z (raw, before mean subtraction)
            surface_y_idx = ref_probe_info['y_idx']
            p_vals = compute_pressure_all_z(p_data, surface_y_idx)

            # Store iteration and surface pressure
            timeseries_data['iterations'].append(iteration)
            timeseries_data['ref_point']['p_z'].append(p_vals)
    ...
    timeseries_data['ref_point']['p_z'] = np.array(timeseries_data['ref_point']['p_z'])
    ...
    # Compute temporal means and fluctuations (now call them p_prime and u_prime)
    print("Computing fluctuations...")
    ref_p_mean = np.mean(timeseries_data['ref_point']['p_z'], axis=0)
    timeseries_data['ref_point']['p_prime'] = timeseries_data['ref_point']['p_z'] - ref_p_mean
...
if 'time' not in timeseries_data:
    ...
    if 'iterations' in timeseries_data and isinstance(timeseries_data['iterations'], np.ndarray):
        ...
        ref_data_shape = timeseries_data['ref_point']['p_z'].shape
...
z_idx = 0
p_prime_z0 = timeseries_data['ref_point']['p_prime'][:, z_idx]
...
ax_tau.plot(time, p_prime_z0, linewidth=0.8, alpha=0.85,
...
ax_tau.set_ylabel(r"$p^\prime$ (Pa)", fontsize=11, fontweight='bold')
ax_tau.set_title(r'Pressure Fluctuation $p^\prime$ (Surface, z-index = 0)',
...
if True:
    ...
    for iz in range(nz):
        p_z = timeseries_data['ref_point']['p_prime'][:, iz]

        # Preprocess signal using global DETREND_TYPE configuration
        p_preprocessed = preprocess_signal_for_welch(p_z, detrend_type=DETREND_TYPE)

        # PSD for pressure (autospectrum)
        f_psd, S_pp_z = signal.welch(
            p_preprocessed,
            fs=fs,
            window=WINDOW,
            nperseg=nperseg,
    ...
    S_pp_all_z = np.array(S_pp_all_z)
    S_pp_mean = np.mean(S_pp_all_z, axis=0)
    ...
    for i, probe in enumerate(probe_info):
        ...
        for iz in range(nz):
            ...
            f_spec, S_pp_z, S_uu_z, S_pu_z = compute_spectra_welch(
                p_z, u_z, fs, window=WINDOW, nperseg=nperseg, noverlap=noverlap, detrend=DETREND_TYPE
            )

            S_pp_all_z.append(S_pp_z)
            S_uu_all_z.append(S_uu_z)
            S_pu_all_z.append(S_pu_z)
        ...
        coherence_all_z_local = np.abs(S_pu_all_z)**2 / (S_pp_all_z * S_uu_all_z)
        coherence_mean_local = np.mean(coherence_all_z_local, axis=0)

        # CSD magnitude and phase from averaged spectra
        csd_mag_mean = np.abs(S_pu_mean)
        csd_phase_mean = np.angle(S_pu_mean)  # Phase in [-pi, pi]
        ...
        spectral_data[f'probe_{i}'] = {
            ...
            'S_pp_mean': S_pp_mean,
            'S_uu_mean': S_uu_mean,
            'S_pu_mean_real': np.real(S_pu_mean),
            'S_pu_mean_imag': np.imag(S_pu_mean),
        ...
        if i == 0:
            ...
            print(f"    S_pp_mean shape: {S_pp_mean.shape}")
    ...
    # 1. Surface pressure
    p_val = timeseries_data['ref_point']['p_prime'][:, z_val]
    validation_signals.append(('surface_pressure', p_val, 'Surface p_prime'))
...
ax.loglog(f_nd[freq_start:], spectral_data['ref_point']['psd'][freq_start:],
         'o-', linewidth=1.5, markersize=3, label="p' (Surface)",
...
ax.set_title(f"Coherence: p' vs u' (z-averaged over full span, nz={nz})", fontsize=13, fontweight='bold')
ax.set_ylim([0, 1.05])
ax.grid(True, which='both', alpha=0.3, linestyle='-', linewidth=0.5)
ax.grid(True, which='minor', alpha=0.15, linestyle=':', linewidth=0.3)
...
ax.set_xlabel("Nondimensional Frequency f* = f·c/U∞ [-]", fontsize=12, fontweight='bold')
ax.set_ylabel("|S_pu(f)| [signal product / Hz]", fontsize=12, fontweight='bold')
ax.set_title(f"Cross-Spectrum Magnitude: p' vs u' (z-averaged, nz={nz})",
            fontsize=13, fontweight='bold')
ax.grid(True, which='both', alpha=0.3, linestyle='-', linewidth=0.5)
ax.grid(True, which='minor', alpha=0.15, linestyle=':', linewidth=0.3)
ax.legend(fontsize=10, loc='upper right', framealpha=0.9)
...
print(f"  4. Z-averaged coherence (p' vs each probe)")
print("="*70)
